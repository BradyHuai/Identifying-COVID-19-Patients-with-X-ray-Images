...
data_dir = '/content/DLAI3_Phase3/DLAI3_Phase3'
data_dir = correction(data_dir)


# 0:Covid 1: NoFinding 2: Thorax
def dataload(batch_size=20,data_dir):
    ...
    dataset = torch.utils.data.Subset(dataset, np.random.choice(len(dataset), 512, replace=False))

    print("Total number of images loaded:", len(dataset))
    train, valid, test = torch.utils.data.random_split(dataset, [round(0.7*len(dataset)),round(0.2*len(dataset)),len(dataset)-round(0.7*len(dataset))-round(0.2*len(dataset))])
    ...
    # train, test = torch.utils.data.random_split(dataset, [4406,1101])
    # trainLoader = torch.utils.data.DataLoader(train, batch_size = batch_size, sampler = sampler)
